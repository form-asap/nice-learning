...
/**
 * Block edit form for block_nice_boxes_2.
 *
...
class block_nice_boxes_2_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        ...
        $mform->addElement(
            'text',
            'config_main_title',
            get_string('config_main_title', 'block_nice_boxes_2')
        );
        ...
        $titleplacementoptions = [
            0 => get_string('left', 'block_nice_boxes_2'),
            1 => get_string('right', 'block_nice_boxes_2'),
            2 => get_string('center', 'block_nice_boxes_2'),
        ];
        $mform->addElement(
            'select',
            'config_title_placement',
            get_string('config_title_placement', 'block_nice_boxes_2'),
            $titleplacementoptions
        );
        ...
        $mform->addElement(
            ...
            get_string('config_main_description', 'block_nice_boxes_2')
        );
        ...
        $mform->addElement(
            ...
            get_string('config_background_image', 'block_nice_boxes_2'),
            null,
            $filemanageroptions
        );

        // Loop for creating settings of three items.
        for ($i = 1; $i <= 3; $i++) {
            // Add a header for each item.
            $mform->addElement(
                'header',
                'config_item' . $i,
                get_string('config_item', 'block_nice_boxes_2') . $i
            );
            ...
            $mform->addElement(
                'text',
                'config_title' . $i,
                get_string('config_title', 'block_nice_boxes_2', $i)
            );
            ...
            $mform->addElement(
                'filemanager',
                'config_image' . $i,
                get_string('config_image', 'block_nice_boxes_2', $i),
                null,
                $filemanageroptions
            );
        }
    ...
    public function set_data($defaults) {
        ...
        if (!empty($this->block->config) && is_object($this->block->config)) {
            // Loop for setting default image data of three items.
            for ($i = 1; $i <= 3; $i++) {
                $field = 'image' . $i;
            ...
            // Logic for setting default background image.
            $backgroundfield = 'background_image';
            $confbackgroundfield = 'config_background_image';
            $draftitemid = file_get_submitted_draft_itemid($confbackgroundfield);
            ...
            // Assign the draft item id to the default data.
            $defaults->{$confbackgroundfield} = ['itemid' => $draftitemid];

            // Update the block's config with the draft item id.
            $this->block->config->{$backgroundfield} = $draftitemid;
        }
