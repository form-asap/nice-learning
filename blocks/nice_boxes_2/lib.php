...
/**
 * Standard functions for block_nice_boxes_2.
 *
...
function block_nice_boxes_2_pluginfile(
    $course,
    $birecordorcm,
    $context,
    $filearea,
    ...
    // This check allows for both 'items' and 'background'.
    if ($filearea !== 'items' && $filearea !== 'background') {
        send_file_not_found();
    }
    ...
    // Only one call to get_file() is needed. It handles both 'items' and 'background' due to previous checks.
    $file = $fs->get_file(
        $context->id,
        'block_nice_boxes_2',
        $filearea,
