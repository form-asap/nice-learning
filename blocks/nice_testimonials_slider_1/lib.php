...
/**
 * Standard functions for block_nice_testimonials_slider_1.
 *
...
function block_nice_testimonials_slider_1_pluginfile(
    $course,
    $birecordorcm,
    $context,
    $filearea,
    ...
    // If the block is in a course context, then check if the user has capability to access the course.
    if ($context->get_course_context(false)) {
        require_course_login($course);
    } else if ($CFG->forcelogin) {
        require_login();
    } else {
        // Get the parent context and check if the user has proper permissions.
