...
/**
 * Class definition for the block_nice_boxes_4.
 *
 * @package     block_nice_boxes_4
 * @copyright   2025 Nice Learning <support@docs.nicelearning.org>
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_nice_boxes_4 extends block_base {
    ...
    public function init(): void {
        $this->title = get_string('pluginname', 'block_nice_boxes_4');
    }
    ...
    public function specialization(): void {
        ...
        if (empty($this->config)) {
            ...
            for ($i = 1; $i <= 4; $i++) {
                $subtitlefield = "sub_title{$i}";
                if (!isset($this->config->$subtitlefield)) {
                    $this->config->$subtitlefield = $defaultsubtitles[$i];
                }
            }
        }
    }
        ...
        for ($i = 1; $i <= 4; $i++) {
            ...
            if (!empty($this->config->$image) && count($files) >= 1) {
                ...
                $imgsrc = moodle_url::make_file_url(
                    ...
                    "/{$this->context->id}/block_nice_boxes_4/items/$i/$mainfilename"
                );
            }
            ...
            $subtitle = isset($this->config->{"sub_title{$i}"})
                ? format_text(
                    $this->config->{"sub_title{$i}"},
                    FORMAT_HTML,
                    ['filter' => true]
                )
                : '';
            ...
            $this->content->text .= '
                <div class="col-sm-6 col-md-6 col-lg-6 col-xl-3">
                    <div class="nice-box-container nice-background-white nice-border-radius">
                        <div class="nice-box-sub-title-container text-center">
                            <p class="m-0">' . $subtitle . '</p>
                        </div>
                        <div class="nice-box-image-container d-flex align-items-center justify-content-center rounded-circle mx-auto">
                            <img src="' . $imgsrc . '" alt="' . $title . ' ' . $i . '" />
                        </div>
                        <div class="nice-box-title-container">
                            <p class="nice-box-title fw-bold m-0">'
                                . $title . '
                            </p>
                        </div>
                    </div>
