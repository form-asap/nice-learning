...
/**
 * Standard functions for block_nice_boxes_1.
 *
...
function block_nice_boxes_1_pluginfile(
    $course,
    $birecordorcm,
    $context,
    $filearea,
    ...
    // If block is in course context, then check if user has capability to access the course.
    if ($context->get_course_context(false)) {
        require_course_login($course);
    } else if ($CFG->forcelogin) {
        require_login();
    } else {
        // Get parent context and see if user has proper permission.
        ...
        if ($parentcontext->contextlevel === CONTEXT_COURSECAT) {
            ...
            // The block is in the context of a user. It is only visible to the user it belongs to.
            send_file_not_found();
        }
